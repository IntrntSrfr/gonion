...
// StartListen will start the Node listener at a given port.
...
// handle is the handler for the listener. It will listen for packets. The first packet MUST be an RSA encrypted
// Packet with an Ask header, otherwise it will close the connection. It will then do a key exchange with an incoming
// client. Every packet afterwards will be AES decrypted and sent to their appropriate Packet Type handler
...
// processData processes a Packet that has a data header. it takes in the first already checked packet, then
// keeps reading from the connection until there are no more data packets. It will then do an HTTP request
// that is decided by the incoming data, then sends it back to the connecting node.
...
func (h *Node) processRelay(c net.Conn, f *packet.Packet, key []byte) {
	...
	defer h.closeConnection(ip.To4().String(), port)
...
func (h *Node) getConnection(ip, port string) (net.Conn, error) {
	...
	h.connections[cStr] = c
	return c, nil
}

// closeConnection closes and removes a connection from the pool
func (h *Node) closeConnection(ip, port string) {
	h.Lock()
	defer h.Unlock()
	cStr := fmt.Sprintf("%v:%v", ip, port)
	delete(h.connections, cStr)
}

// closeConn tries to close a net.Conn, and logs any errors
...
// createSecret takes in a client secret, makes a node secret, combines them and returns them.
